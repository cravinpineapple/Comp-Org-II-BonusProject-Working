...
///******************************************************************************************
//*   Prints the c-string starting w/ address of first char.                                *
//******************************************************************************************/
//	  int weight = 1;
//	  int total = 0;
//	  
...
//	  regAC.postActionEvent();
	  ...
	  // get address passed from operand 
	  int memoryCol, memoryRow;
	  int currAddress = regMAR.getValue();
	  int printVal = 1;
	  
	  while (true) {
		  // get the hex value from where the address passed is located
		  memoryRow = currAddress / 16;
		  memoryCol = currAddress % 16 + 1;
		  
		  // convert hex value to int
		  printVal = stringToInt(HEX, memoryArray[memoryRow][memoryCol].toString());
		 
		  // check if this is the end of the c-string
		  if (printVal == 0)
			  break;
		  
		  // output to MARIE console the number (ASCII value)
		  regOUTPUT.setValue(printVal);
		  String outStr = regOUTPUT.toString().trim();
		  outputStream.addElement(new Integer(regOUTPUT.getValue()));
		  if (regOUTPUT.toString() != null)
			  outputArea.append(outStr);
		  if (outputWithLinefeed)
		  ...
		  // increment currAddress to look at next ASCII in c-string
		  currAddress++;  
	  }
  } // print()
